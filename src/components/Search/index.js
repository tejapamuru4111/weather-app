...
class Search extends Component {
    ...
    getWeather = async() => {
        ...
        const {input} = this.state
           ...
           //console.log(data)
           if(response.ok) {
                const response2 = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${input}&appid=${apiKey}`)
                const data2 = await response2.json()
                //console.log(forecastArr)
                if (response2.ok) {
                    const localStorageFavPlaces = localStorage.getItem('fav_weather_places')
                    const favPlaces = localStorageFavPlaces === null? [] : JSON.parse(localStorageFavPlaces)
                    const forecastArr = data2.list.filter((obj,i) => i%8 === 0)
                    //console.log(favPlaces)
                    this.setState({dataObj: data, forecast:forecastArr, api: apiConstants.success, favList: favPlaces})
                }
                else {
    ...
    renderCurrentWeather = () => {
        ...
        return (
            <>
                ...
                {isPresent? 
                <button type='button' onClick={this.onClickRemove} className='add-remove-favorite'>Remove Favorite</button>                
                : 
                <button type='button' onClick={this.onClickAdd} className='add-remove-favorite'>Add to Favorite</button>}
            </>
        )
    }
        ...
        return (
            <div className='weather-home-page-bg-container'>
                <div className='weather-home-page-bg-card'>
                    <input type='search' className='location-search-bar' value={input} placeholder="Enter place or city or State" onChange={this.onChangeInput} onKeyDown={this.onKeyDown}/>
                    <button type='button' className='search-location-button' onClick={this.onClickSearch}>Search</button>
                    <div className='link-to-home-card'>
                        <Link to='/' className='link-to-home-page'>Home</Link>
                    </div>
                    {this.renderAllPages()}
                </div>
            </div>
