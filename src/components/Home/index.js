...
class Home extends Component {
    ...
    getWeather = async(lat,long) => {
           ...
           if(response.ok) {
                const response2 = await fetch(`https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${long}&appid=${apiKey}`)
                ...
                if (response2.ok) {
                    ...
                    this.setState({dataObj: data, forecast:forecastArr, favoritePlacesList: favPlaces, apiStatus: apiConstants.success})
                }
                else {
        ...
        return (
            <>
                ...
                <ul className='home-page-favorite-places-list'>
                    {favoritePlacesList.length !== 0? favoritePlacesList.map(place => (
                        <li key={place.id}>
                            <Link to={`/search/${place.name}`} className='home-page-favorite-places-list-item'>{place.name}</Link>
                        </li>
                    )): <p className='home-page-favorite-places-list-empty'>No Favorite Places</p>}
                </ul>
            </>
        )
    }
            ...
            <div className='weather-home-page-bg-container'>
                <div className='weather-home-page-bg-card'>
                    <div className='home-page-nav-bar'>
                        ...
                        <Link to='/search'><button type='button' className='home-search-button'><BiSearch /></button></Link>
                    </div>
